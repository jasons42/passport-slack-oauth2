...
function Strategy(options, verify) {
  options = options || {};
  options.tokenURL = options.tokenURL || 'https://slack.com/api/openid.connect.token';
  options.authorizationURL = options.authorizationURL || 'https://slack.com/openid/connect/authorize';
  options.scope = options.scope || ['openid', 'email', 'profile'];

  this.profileUrl = options.profileURL || options.profileUrl || "https://slack.com/api/openid.connect.userInfo"; // requires 'openid' scope
  this._team = options.team;

  console.debug("passport-slack-oauth2: options: " + JSON.stringify(options));
  ...
  if(!this._skipUserProfile && this._scope.indexOf('openid') === -1){
    console.warn("Scope 'openid' is required to retrieve Slack user profile");
  }
}
...
Strategy.prototype.userProfile = function(accessToken, done) {
  ...
  this.get(this.profileUrl, header, function (err, body, res) {
    if (err) {
      console.error("passport-slack-oauth2: error: " + err);
      return done(err);
    } else {
      try {
        var profile = JSON.parse(body);
        console.debug("passport-slack-oauth2: parsed profile: " + JSON.stringify(profile));
        ...
        if (!profile.ok) {
          ...
          profile.id = profile['https://slack.com/user_id'] || profile.user.id;
          profile.displayName = profile.name || profile.user.name;

          console.debug("passport-slack-oauth2: returning profile: " + JSON.stringify(profile));
          done(null, profile);
        }
      } catch(e) {
        done(e);
...
Strategy.prototype.authorizationParams = function (options) {
  ...
  console.debug("passport-slack-oauth2: authorizationParams: " + JSON.stringify(params));
  return params;
};
